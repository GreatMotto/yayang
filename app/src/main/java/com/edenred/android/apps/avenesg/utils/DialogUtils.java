    ...
    public static void congratulationDialog(final Context mContext) {
        ...
        tv_btnText.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ((BaseActivity) mContext).goto1OtherActivity(MyProfileActivity.class, 8);
            }
        });
...
//                textdata = month1 + "/" + day1 + "/" + year;
     ...
     * @param flag     显示按钮个数
     */
    public static void ProfileDlg(final Context mContext, String title, String content, final int flag, final int tag, final int fromtag) {
        final Dialog alertDialog = new Dialog(mContext, R.style.MyDialogStyle);
        Window window = alertDialog.getWindow();
        window.setGravity(Gravity.CENTER);
        alertDialog.show();
